...
sim_results = tibble(true_mu = c(0,1,2,3,4,5,6)) %>% 
  mutate(
    output = map(.x = true_mu, ~rerun(100, simulation(mu = .x))),
...
```
It is clear that as the true value of mu or effect size increases, power increases as well. Hence power and effect size are directly related. 
